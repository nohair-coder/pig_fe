    ...
    <a-card :bordered="false">
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          <template>
            <a @click="handleEdit(record)">转栏</a>
          </template>
        </span>
      </s-table>
...
<script>
  ...
  import { changeStationId, queryAllList } from '@/api/pig'
        ...
        form.validateFields((errors, values) => {
          if (!errors) {
            ...
            if (values.stationId) {
              ...
              // 修改earId
              changeStationId(values).then(res => {
                this.visible = false
                this.confirmLoading = false
                // 刷新表格
                this.$refs.table.refresh()

                this.$message.success('转栏成功！')
              }).catch(() => {
                this.visible = false
                this.confirmLoading = false
                // 刷新表格
